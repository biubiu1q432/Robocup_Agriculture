...
class ROI:
    def __init__(self, roi, roi_frame, roi_name=None):
        ...
        self.roi_frame = roi_frame#当前ROI的图像数据
        self.roi_filtered_frame = None  # 用来保存并看结果的，不是用来处理的
        self.roi_name = roi_name    # 默认没有名字（可选）

        self.obj_center_coord = (-1, -1)#目标物体的中心坐标
        self.obj_shape = (-1, -1)#目标物体的形状
        self.is_ripe = const.NOVEGE#目标物体的成熟度（默认为const.NOVEGE，表示未检测到植物）
